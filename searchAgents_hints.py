...
class CapsuleSearchProblem:
    ...
    def getStartState(self):
        ...
        return (self.init_pos, self.foodGrid, self.capsulesGrid)
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            next_x, next_y = int(x + dx), int(y + dy)
            if not self.walls[next_x][next_y]:
                next_foods = foods.copy()
                next_capsules = capsules.copy()
                cost = 1
                if foods[next_x][next_y]:
                    next_foods[next_x][next_y] = False
                if capsules[next_x][next_y]:
                    next_capsules[next_x][next_y] = False
                    cost = 0
                successors.append( ( ((next_x, next_y), next_foods,next_capsules), direction, cost) )
        return successors
...
def capsuleProblemHeuristic(state, problem):
    ...
    return cPH1(state, problem)



def cPH0(state, problem):
    """
    Your heuristic for the CapsuleSearchProblem goes here.
...
def cPH1(state,problem):
    pos,foodGrid,capsules = state
    return foodGrid.count()

def cPH2(state,problem):
    pos,foodGrid,capsules = state
    maxDis = 0
    if len(foodGrid.asList()) ==0:
        return 0
    for food in foodGrid.asList():
        dis = util.manhattanDistance(pos,food)
        red = capsuleReduction(pos,food,capsules)
        # print(pos,food,dis,red,capsules.asList())
        maxDis = max(dis-red,maxDis)
    h= max(maxDis,foodGrid.count())
    # print(state)
    # print(capsules.asList())
    # print(h)
    return h

def capsuleReduction(pos,target,capsules):
    x,y = pos
    x_1,y_1 = target
    counter = 0
    # print(f"height: {capsules.height}")
    # count the row that contains capsule
    x_max = max(x,x_1)
    x_min = min(x,x_1)
    for i in range(x_min,x_max+1):
        for j in range(capsules.height):
            # print(f"{i},{j}:{capsules[i][j]}")
            if capsules[i][j]:
                counter += 1
                break
    
    # count the column that contains capsule
    y_max = max(y,y_1)
    y_min = min(y,y_1)
    for j in range(y_min,y_max+1):
        for i in range(capsules.width):
            if capsules[i][j]:
                counter += 1
                break
            
    # avoid same cell counted in row and column for both
    counter = min(counter-1,capsules.count())
...
def cPH3(state,problem):
    pos,foodGrid,capsules = state
    maxDis = 0
    if len(foodGrid.asList()) ==0:
        return 0
    for food in foodGrid.asList():
        maxDis = max(CMD(state,food,problem),maxDis)
    h = max(maxDis, foodGrid.count())
    # print(f"{pos}; {foodGrid.asList()}; {h}")
    return h
    ...
    while not myQ.isEmpty():
        ...
        if state[0]==target:
            ...
            for next_state in set(expand(state,problem)):
                ...
                if not next_state in generated:
...
def expand(state,problem):
    ...
    for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
        ...
        if not problem.walls[next_x][next_y]:
            ...
            if capsules[next_x][next_y]:
                next_capsules[next_x][next_y] = False
                successors = successors + expand(((next_x, next_y), next_foods,next_capsules),problem)
            else:
